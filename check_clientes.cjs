...
async function check() {
  try {
    const res = await pool.query("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'clientes';");
    console.log('Clientes Columns:', res.rows);
  } catch (err) {
    console.error(err);
  } finally {
    await pool.end();
