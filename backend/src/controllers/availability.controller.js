...
export const listAvailability = async (req, reply) => {
  const availability = await getAvailabilityByBusiness(req.business.id)
...
export const setAvailability = async (req, reply) => {
  ...
  const result = await upsertAvailability(req.business.id, day_of_week, start_time, end_time)
  reply.send(result)
}
...
export const removeAvailability = async (req, reply) => {
  ...
  const deleted = await deleteAvailability(req.business.id, day)
  if (!deleted) {
